...
| TM-023 | Audit tampering/omission | `tests/integration/audit.spec.ts` | PLANNED | 2026-08-13 local DB evidence: typed payload rejection, full-event timestamp/correlation/hash verification, row-bound DB hash guard, transactional rollback, sequence continuity, retry de-duplication, append-only trigger; integrated suite remains | WS-003/007 |
...
| P1-004 | Atomic PostgreSQL reservation invariant | PASS | 2026-08-13: `npm run test:db` 25/25, `npm run test:concurrency` 1/1 across 32 rounds, `npm run test:invariants` 7/7 with seeds 2026081301/2026081302/2026081303; loopback PostgreSQL 17 on 127.0.0.1:55432 |
| P1-005 | Phase-1 integration review and shared-contract freeze | BLOCKED | 2026-08-13: local plans, risks, matrix, workstreams, changelog, migration state, final rows, property seeds, and an independent review with addressed findings reconciled; Gate S1 still requires protected CI, authenticated adapter/reconciler integration, and wider authorization/revocation evidence | Lead |
