...
| R-033 | Chain facts change after preflight while signing waits for DB fence locks | High | **OPEN / BLOCKING P3-01:** a DB deadline detects elapsed time, not Anvil head/nonce/balance/fee changes. ADR-0018 also forbids RPC under the held locks. No DB-only mechanism observes this external mutation; product-owner decision required before P3-02 |
