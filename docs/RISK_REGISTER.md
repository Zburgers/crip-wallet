...
| R-003 | Concurrent reservations overspend / integrity | High | Critical | Serializable transaction, constraints, concurrency/property proof | WS-003 | 1 | Open |
| R-004 | Retry duplicates reservation or execution / reliability | High | Critical | Unique idempotency, durable state, bounded retry | WS-003 | 1 | Open |
...
| R-009 | Destructive migration loses financial state / data | Low | Critical | Forward-only review, backup/restore and correction drills | WS-003 | 1 | Open |
...
| R-013 | Audit event missing from state transition / evidence | Medium | High | Same DB transaction and rollback tests | WS-003 | 1 | Open |
