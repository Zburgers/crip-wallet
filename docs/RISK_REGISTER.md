...
| R-030 | Pause/revoke after signed evidence or an alternate raw sender still reaches broadcast | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** P3-02 migrations `0027`–`0030` quarantine signed/no-attempt work, fence a new `STARTED` attempt, block reuse of an invalidated REJECTED attempt, and prevent its direct release/expiry; P3-03 still owns full authority revalidation and sole-sender proof before closure |
| R-031 | Post-send fence change strands or falsely releases economic effect | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** `0028` and DB regressions allow exact ACCEPTED/UNKNOWN reconciliation after control while `0029` blocks REJECTED no-send broadcast/finalization and `0030` blocks its direct release/expiry; P3-03/04 still own `STARTED` crash recovery, lease fencing, ambiguity, and exactly-once proof |
