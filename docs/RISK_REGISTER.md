...
| R-033 | Chain facts change after preflight while signing waits for DB fence locks | High | **IMPLEMENTED LOCALLY / REVIEW PENDING:** the loopback RPC gateway serializes supported Anvil mutations and lifecycle actions with the signer's lease; signing acquires the lease before its final freshness RPC and holds it through evidence commit. Anvil is unpublished on an internal network, PostgreSQL is isolated, and the gateway checkpoints mutations fail-closed. Close after fresh independent review and exact-SHA CI; host Docker privileges remain inside the local trust boundary. |
