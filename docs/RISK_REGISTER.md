...
| R-030 | Pause/revoke after signed evidence or an alternate raw sender still reaches broadcast | Critical | **MITIGATED LOCALLY / P3-04:** P3-02/03 enforce quarantine and sole-gateway `STARTED` authority; obsolete sign-only child sources are removed, the package build clears stale artifacts, and the capability test proves the public API routes through the execution child. MAX review and exact-head CI/Secret Scan pass; P3-06 closeout remains |
| R-031 | Post-send fence change strands or falsely releases economic effect | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** P3-03 recovery verifies exact canonical evidence for ACCEPTED/UNKNOWN/CONFLICT and mined STARTED attempts after control, with one economic effect; invalidated REJECTED attempts remain blocked by `0029`/`0030`. P3-04 adds DB-time final lease fencing, exact signed-no-attempt release, and deterministic lock-order races; 162 DB tests and Phase-3 gate 254/254 pass. MAX review and exact-head CI/Secret Scan pass; P3-05 local matrix passes; P3-06 closeout remains |
| R-032 | Signer/control lock inversion deadlocks or selects stale authority | High | P3-01 canonical fence-first order and deterministic two-winner proof pass locally. P3-04 deterministic PostgreSQL barrier proves control locks authorization before operation rows; MAX review and exact-head CI/Secret Scan pass (`54afbcc9`, `35787705165`, `35787705289`) |
