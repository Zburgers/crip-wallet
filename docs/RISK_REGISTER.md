...
| R-030 | Pause/revoke after signed evidence or an alternate raw sender still reaches broadcast | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** P3-02 migrations `0027`/`0028` quarantine signed/no-attempt work and fence a new `STARTED` attempt; P3-03 still owns full authority revalidation and sole-sender proof before closure |
| R-031 | Post-send fence change strands or falsely releases economic effect | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** `0028` and a DB regression allow exact ACCEPTED-attempt reconciliation after control; P3-03/04 still own `STARTED` crash recovery, lease fencing, ambiguity, and exactly-once proof |
