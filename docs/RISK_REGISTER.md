...
| R-015 | Stale worker acts after lease loss | Critical | Authenticated bounded recovery leases; **mitigated for local S1**. P3-04 uses DB time and exact credential/version for renewal and final resolution; expiry/takeover and final-write rollback tests pass; follow-up MAX review passes; exact-head CI/Secret Scan pending |
...
| R-020 | Dependency security-update drift | Medium | Dependabot + audit CI; current `npm audit --audit-level=high` reports no high/critical issues and two moderate Vitest/@vitest-mocker advisories; a fix is available in Vitest 4.1.11 outside the declared range, so the update remains monitored |
...
| R-030 | Pause/revoke after signed evidence or an alternate raw sender still reaches broadcast | Critical | **MITIGATED LOCALLY / P3-04:** P3-02/03 enforce quarantine and sole-gateway `STARTED` authority; obsolete sign-only child sources are removed, the package build clears stale artifacts, and the capability test proves the public API routes through the execution child. Follow-up MAX review passed; exact-head CI/Secret Scan and P3-05/06 evidence remain |
| R-031 | Post-send fence change strands or falsely releases economic effect | Critical | **PARTIALLY MITIGATED LOCALLY / OPEN:** P3-03 recovery verifies exact canonical evidence for ACCEPTED/UNKNOWN/CONFLICT and mined STARTED attempts after control, with one economic effect; invalidated REJECTED attempts remain blocked by `0029`/`0030`. P3-04 adds DB-time final lease fencing, exact signed-no-attempt release, and control/recovery/envelope-replacement plus approval, autonomous, signer, and broadcaster lock-order races; 162 DB tests and the Phase-3 gate 254/254 pass. Follow-up MAX review passed; exact-head CI/Secret Scan and P3-05/06 closeout remain |
| R-032 | Signer/control lock inversion deadlocks or selects stale authority | High | P3-01 canonical fence-first order and deterministic two-winner proof pass locally. P3-04 adds a deterministic PostgreSQL barrier proving control locks authorization before operation rows; follow-up MAX review passed; exact-head CI/Secret Scan pending |
