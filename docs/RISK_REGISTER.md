...
| R-030 | Pause/revoke after signed evidence or an alternate raw sender still reaches broadcast | Critical | **OPEN:** P3-02/03 must replace the invalidation trigger, quarantine signed/no-attempt work, make one restricted `STARTED`-gated sender, and revalidate full current authority before send; ADR-0018 proposed |
